...
g_init  <- ggplot(
  ...
  aes(
    ...
    scale_fill_brewer( name = "RNA-seq condition",
...
plot(g_init)
ggsave(filename = (paste("Initial_call.png")), plot = g_init, width = 8, height = 4, dpi = 300)
...
g_filt <- ggplot(
  ...
  scale_fill_brewer("RNA-seq condition",
                    breaks=c( "Recovery_3h", "Recovery_80min", "Tun", "Active" )) +
  labs(
...
ggsave(filename = (paste("Final_Filted_call.png")), plot = g_filt, width = 8, height = 4, dpi = 300)
